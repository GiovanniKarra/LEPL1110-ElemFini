/*
 *  glfem.c
 *  Library for LEPL1120 : Finite Elements for dummies
 *
...
void glfemPlotSolver(femBandSystem *mySolver, int n, int w, int h)
{
    double sizeX = (n-1)*40/1.7;
    double meanX = (n-1)*20; 
    double sizeY = (n-1)*40/1.7;
...
void glfemPlotField(femMesh *theMesh, double *u)
{
    ...
    float  yLoc[4];
    double uLoc[4];
    double uMax = femMax(u,theMesh->nodes->nNodes);
    double uMin = femMin(u,theMesh->nodes->nNodes);
